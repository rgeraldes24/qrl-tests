...
func (parameters *consoleParameters) buildStoreCase(
	...
	nestedData, err := deployment.abi.Pack("roundTrip", nestedValues)
	if err != nil {
		return fmt.Errorf("pack nested array call: %w", err)
	}
	...
	parameters.NestedData = hexutil.Encode(nestedData)
	return nil
}
