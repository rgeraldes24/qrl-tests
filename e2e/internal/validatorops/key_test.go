...
func TestDeterministicKeyIsStable(t *testing.T) {
	...
	require.Len(t, first.PublicKey(), signing.PublicKeyLength)
...
func TestSignaturesVerify(t *testing.T) {
	...
	var message signing.Root
	message[0] = 1
	signature, err := key.Sign(message[:])
	require.NoError(t, err)
	require.Len(t, signature, signing.SignatureLength)
	require.NoError(t, signing.Verify(message, key.PublicKey(), signature))

	message[0] = 2
	require.Error(t, signing.Verify(message, key.PublicKey(), signature))
}
