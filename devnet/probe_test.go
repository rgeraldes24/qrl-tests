...
func TestProbeNetwork(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "qrl_getBalance":
			require.Equal(t, []string{DevelopmentWalletAddress, "latest"}, payload.Params)
	...
	require.NoError(t, probeNetwork(context.Background(), server.URL, DevelopmentWalletAddress))
