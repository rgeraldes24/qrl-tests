...
import (
	...
	"github.com/cyyber/qrl-tests/e2e/internal/depositcli"
...
const (
	...
	// a 16-minute genesis voting floor.
	...
	// userKeyMarker seeds the withdrawal recipient. It must not collide
	// with genesis validators or the protocol-suite staker marker 0x91.
	userKeyMarker = 0x92
)

func TestE2E(t *testing.T) {
	testsuite.Run(t, "Staker user-flow E2E suite")
}
...
var _ = ginkgo.Describe(
	...
	ginkgo.Serial,
	ginkgo.Ordered,
	ginkgo.Label("e2e", "consensus", "staker-cli", "mutates-chain"),
	func() {
		var (
			node         *live.Node
			chain        consensuscontext.Context
			operator     *operatorvc.Validator
			deposit      depositcli.Result
			publicKey    string
			maximum      uint64
		...
		ginkgo.BeforeAll(func(ctx ginkgo.SpecContext) {
			...
			recipientKey := testsuite.MustSucceed(validatorops.DeterministicKey(userKeyMarker))
			...
			deposit = testsuite.MustSucceed(depositcli.Run(ctx, depositcli.Config{
				Image:               depositcli.ImageFromEnv(),
			...
			gomega.Expect(deposit.Amount).To(gomega.Equal(maximum),
				"deposit CLI amount %d does not match MAX_EFFECTIVE_BALANCE %d", deposit.Amount, maximum)

			_, err := node.Beacon.Validator(ctx, publicKey)
			gomega.Expect(beacon.IsNotFound(err)).To(gomega.BeTrue(), "user staker key is already a validator: %v", err)
			...
			gomega.Expect(containsPublicKey(keystores, publicKey)).To(gomega.BeTrue(),
		...
		ginkgo.It("deposits the maximum balance through the deposit CLI", func(ctx ginkgo.SpecContext) {
			...
			gomega.Expect(strings.EqualFold(validator.PublicKey, publicKey)).To(gomega.BeTrue())
			...
			gomega.Expect(strings.EqualFold(validator.RandaoCommitment, deposit.RandaoCommitment)).To(gomega.BeTrue(),
				"validator record carries a different RANDAO commitment than the deposit CLI")
		}, ginkgo.SpecTimeout(initialDepositTimeout))
		...
		ginkgo.It("exits the validator through the accounts CLI and withdraws its stake", func(ctx ginkgo.SpecContext) {
			...
			ginkgo.By("signing the voluntary exit with validator accounts voluntary-exit")
			...
			gomega.Expect(operator.VoluntaryExit(ctx)).To(gomega.Succeed())
