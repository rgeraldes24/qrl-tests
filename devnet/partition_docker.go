...
func (partition *dockerNetworkPartition) Apply(ctx context.Context, groups ...[]Participant) error {
	...
	containerID := func(serviceID string) (string, error) {
		if container := containers[serviceID]; container != "" {
			return container, nil
		}
		container, err := partition.containerID(ctx, serviceID)
		if err == nil {
			containers[serviceID] = container
		}
			...
			for _, source := range sources {
				for _, destination := range destinations {
					for _, endpoints := range []struct {
						...
						{source.Execution.ID, destination.Execution.PrivateIP},
						{source.Consensus.ID, destination.Consensus.PrivateIP},
					} {
						if endpoints.serviceID == "" || endpoints.peerIP == "" {
							return errors.New("participant is missing a service ID or private IP")
						}
						container, err := containerID(endpoints.serviceID)
						if err != nil {
